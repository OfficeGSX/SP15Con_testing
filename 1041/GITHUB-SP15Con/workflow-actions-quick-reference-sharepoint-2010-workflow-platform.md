...
|現在のアイテムにフィールドを設定する  <br/> | このアクションは、最初はワークフロー ステップで [ **フィールド を 値 に設定する**] として表示されています。このアクションを使用して、現在のアイテムのフィールドに値を設定します。  <br/>  以下にワークフロー ステップ内でのアクションの例を示します。 <br/> コンテンツ タイプの ID を仕様 に設定する <br/> <br/> **メモ**<br/>  フィールドの値が変更されるまでワークフローを停止する場合は、代わりに [ **現在のアイテムでのフィールドの変更を待つ**] アクションを使用します。 <br/>  [ **現在のアイテムにフィールドを設定する**] アクションは、サイト ワークフロー使用時には利用できません。           |
